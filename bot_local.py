...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = "Haritztya Azka Zhaviri M",
            BATCH_ACADEMY = "THEIA",
            GITHUB_REPO_LINK = "https://github.com/haritztya/capstone_da_telebot"
        )
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        plot_info = list(zip(
            ['spent', 'total approved conversion', 'cost per click'],
            [df_plot[df_plot['spent']==df_plot['spent'].max()].index[0],df_plot[df_plot['approved_conversion']==df_plot['approved_conversion'].max()].index[0], df_plot[df_plot['cpc']==df_plot['cpc'].max()].index[0]],
            [df_plot[df_plot['spent']==df_plot['spent'].min()].index[0],df_plot[df_plot['approved_conversion']==df_plot['approved_conversion'].min()].index[0], df_plot[df_plot['cpc']==df_plot['cpc'].min()].index[0]]))
...
def echo_all(message):
    ...
    with open('template_text/default.txt', mode='r', encoding='utf-8') as f:
        ...
        default = temp.substitute(EMOJI = emoji.emojize(':upside-down_face:'))
